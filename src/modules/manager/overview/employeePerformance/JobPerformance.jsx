...
const JobPerformance = () => {
	...
	return (
		<div className='page'>
			...
			>
				<Column
					body={(rowdata) => bodyTable(rowdata?._id)}
					header={() => headerTable('Saler')}
					className='p-0'
				/>
				<Column
					body={(rowdata) => performanceSaler(rowdata?.jobs, JobRules.STATUS_JOBS.INCOMPLETE)}
					header={() => headerTable('Đang yêu cầu')}
				/>
				<Column
					body={(rowdata) => performanceSaler(rowdata?.jobs, JobRules.STATUS_JOBS.PENDING)}
					header={() => headerTable('Đang xử lý')}
				/>
				<Column
					body={(rowdata) => performanceSaler(rowdata?.jobs, JobRules.STATUS_JOBS.COMPLETE)}
					header={() => headerTable('Đã hoàn thành')}
				/>
				<Column
					body={(rowdata) => performanceSaler(rowdata?.jobs, 'total_cost')}
					header={() => headerTable('Doanh thu')}
				/>
				<Column
					body={(rowdata) => performanceSaler(rowdata?.jobs, 'bonus_incomplete')}
					header={() => headerTable('Bonus incomplete')}
				/>
				<Column
					body={(rowdata) => performanceSaler(rowdata?.jobs, 'bonus_complete')}
					header={() => headerTable('Bonus complete')}
				/>
				<Column
					body={(rowdata) => performanceSaler(rowdata?.jobs, 'kpi', rowdata?._id)}
					header={() => headerTable('Kpi')}
				/>
			</DataTable>
			<br />
			<br />
			...
			>
				<Column
					body={(rowdata) => bodyTable(rowdata?._id)}
					header={() => headerTable('Editor')}
					className='p-0'
				/>
				<Column
					body={(rowdata) => performanceEditor(rowdata?.jobs, JobRules.STATUS_JOBS.INCOMPLETE)}
					header={() => headerTable('Đang yêu cầu')}
				/>
				<Column
					body={(rowdata) => performanceEditor(rowdata?.jobs, JobRules.STATUS_JOBS.PENDING)}
					header={() => headerTable('Đang xử lý')}
				/>
				<Column
					body={(rowdata) => performanceEditor(rowdata?.jobs, JobRules.STATUS_JOBS.COMPLETE)}
					header={() => headerTable('Đã hoàn thành')}
				/>
				<Column
					body={(rowdata) => performanceEditor(rowdata?.jobs, 'bonus_complete')}
					header={() => headerTable('Bonus complete')}
				/>
			</DataTable>
		</div>
