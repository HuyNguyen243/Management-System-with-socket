...
import { getCurrentUser } from '../../../../redux/messages/messageSlice';
import { getAllgroups, getAllMembers } from '../../../../redux/messages/messageSlice';
...
const Index = ({
	...
	socket.off('get-members').on('get-members', (payload) => {
		...
		if (user?.data?.role === UserRules.ROLE.EDITOR || user?.data?.role === UserRules.ROLE.LEADER_EDITOR) {
			result = payload.filter((member) => {
				return member?.role === UserRules.ROLE.SALER || member?.role === UserRules.ROLE.ADMIN;
			});
		} else {
			result = payload;
		}
		...
		if (MsgsByIdSystem) {
			...
			MsgsByIdSystem?.forEach((msg) => {
				if (msg.type === NAME_ROOM.USER) {
					const receiver = msg?.members.filter((member) => {
						return member !== currentUser?.id_system;
					});
	...
	return (
		<div
			className={`people-list ${isOpen && 'active'} ${
				currentUser.role !== UserRules.ROLE.ADMIN && 'opacity-0 max-w-0'
			}`}
			id='people-list'
			onClick={handleOpenMembersBlock}
		>
